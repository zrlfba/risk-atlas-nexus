...
class TestLibrary(TestCaseBase):
    ...
    def test_export(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertTrue(os.path.exists(os.path.join(tempdir, "ai-risk-ontology.yaml")))
    ...
    def test_get_risk_by_tag_type(self):
        ...
        ran_lib = self.ran_lib
        risk = ran_lib.get_risk_by_tag("toxic-output")
    ...
    def test_get_related_risks_by_tag(self):
        ...
        ran_lib = self.ran_lib
        risks = ran_lib.get_related_risks_by_atlas_tag("toxic-output")
        self.assertIs(len(risks), 3)

    def test_get_related_risk_ids_by_tag(self):
        """Get related risk definitions from the LinkML, by risk atlas tag"""
        ran_lib = self.ran_lib
        risks = ran_lib.get_related_risk_ids_by_atlas_tag("toxic-output")
        self.assertIs(len(risks), 3)
    ...
    def test_get_risk_actions_by_risk_id(self):
        ...
        ran_lib = self.ran_lib
        risks = ran_lib.get_risk_actions_by_risk_id("nist-human-ai-configuration")
