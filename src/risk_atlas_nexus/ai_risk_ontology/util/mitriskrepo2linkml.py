...
# Local
from risk_atlas_nexus.ai_risk_ontology import Container, Documentation, Risk, RiskGroup, RiskTaxonomy
...
def create_container_object() -> Container:
    ...
    # Create risk groups
    risks = get_risks()
    risk_group_names = sorted(list(set([(risk["Major"], risk["Domain"]) for risk in risks])))
    riskgroups = [
        RiskGroup(
            **{
...
with open(
    "src/risk_atlas_nexus/data/knowledge_graph/mit_ai_risk_repository_data.yaml", "+tw", encoding="utf-8"
) as output_file:
